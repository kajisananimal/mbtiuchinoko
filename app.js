...
function setResultIllustration(type){
  ...
  const t = (type || "").trim();
  const customKey = `uchinoko_mbti_customImage_${t}`;
  const custom = localStorage.getItem(customKey);
  const src = custom || getDefaultTypeImageUrl(t);
  ...
  if(src){
    // assign after handlers (dataURL can load instantly)
    imgEl.src = src;
    // in case it loads synchronously and doesn't trigger onload in some browsers
...
/* ===== RESULT_CATCH_MAP_V12 ===== */
const RESULT_CATCH = {
  INTJ:"静かに燃える、策士タイプ。",
  INTP:"知的好奇心が止まらない、研究者タイプ。",
  ENTJ:"頼れるリーダー、指揮官タイプ。",
  ENTP:"ひらめき王、発明家タイプ。",
  INFJ:"やさしく見守る、共感タイプ。",
  INFP:"ピュアでマイペース、夢見るタイプ。",
  ENFJ:"みんなの味方、応援団タイプ。",
  ENFP:"場を明るくする、きらめきタイプ。",
  ISTJ:"しっかり者、安心ルーティンタイプ。",
  ISFJ:"気配り名人、そっと支えるタイプ。",
  ESTJ:"頼れる番長、テキパキタイプ。",
  ESFJ:"甘え上手、みんな大好きタイプ。",
  ISTP:"クールに器用、職人タイプ。",
  ISFP:"感性きらり、アーティストタイプ。",
  ESTP:"行動力MAX、チャレンジャータイプ。",
  ESFP:"お祭り気分、エンタメタイプ。"
};
...
/* ===== COMPAT_BUTTON_DELEGATION_V10 ===== */
document.addEventListener("click",(e)=>{
  const el = e.target && e.target.closest ? e.target.closest("#btnCompat, #btnChangePhoto, #btnResetPhoto") : null;
  if(!el) return;

  if(el.id === "btnCompat"){
    try{
      const pet = document.getElementById("petTypeSelect")?.value;
      const human = document.getElementById("humanTypeSelect")?.value;
      if(!pet || !human){ toast("タイプを選んでね"); return; }
      const r = calcCompat(pet, human);
      document.getElementById("compatPercent").textContent = `${r.percent}%`;
      document.getElementById("compatTypes").textContent = `${pet}（うちのこ） × ${human}（かいぬし）`;
      document.getElementById("compatText").textContent = r.text;
      const resEl = document.getElementById("compatResult");
      if(resEl) resEl.hidden = false;
      localStorage.setItem("uchinoko_mbti_lastType", pet);
      window.scrollTo({top:document.body.scrollHeight, behavior:"smooth"});
    }catch(err){
      console.error(err);
      toast("相性診断でエラーが出ました。更新して再度お試しください");
    }
  }

  if(el.id === "btnChangePhoto"){
    const t = document.getElementById("resultType")?.textContent?.trim();
    if(t) pickCustomPhoto(t);
  }
  if(el.id === "btnResetPhoto"){
    const t = document.getElementById("resultType")?.textContent?.trim();
    if(t) resetCustomPhoto(t);
  }
});

/* ===== BUTTON_POP_EFFECT_V12 ===== */
document.addEventListener("click",(e)=>{
  const b = e.target && e.target.closest ? e.target.closest("button") : null;
  if(!b) return;
  b.classList.remove("btn-pop");
  // force reflow
  void b.offsetWidth;
